...
import nguyenchithanh.com.appsafe.utils.dialog.showUserAlertDialog
import nguyenchithanh.com.appsafe.utils.fragment.SetUpFrgment
import javax.inject.Inject

class RegisterFragment : Fragment(), Injectable, HasSupportFragmentInjector {
    ...
    private fun setupViewModel() {
        ...
        registerViewModel.resultRegister?.observe(this, Observer { resultRegister ->
            handleDataRespone(resultRegister)
        })
    }
...
//        progressDialogLogin.show()
    ...
    private fun successfulState(user: User) {
         safeSharedPreferences.setUserInfo(user)
        val loginFragment = LoginFragment()
        SetUpFrgment.replaceFragment(context as AppCompatActivity,loginFragment,R.id.containerUser)
    }
    ...
    fun showDialog() {
        userDialog = showUserAlertDialog {
            cancelable = false

            btnYesClickListener {
                registerViewModel.register(UserBody(edt_pass.text.toString().trim()))
            }
